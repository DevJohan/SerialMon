...
public:
	typedef T data_type;
	typedef typename conditional_const<T,is_const>::type reference_data_type;
...
public:
	...
	basic_image_signal( basic_image_signal&& original ):
		...
		_byte_row_step(original._byte_row_step),
		_byte_array( original._byte_array )
		{
			original._byte_array = nullptr;
		}
	...
	raw_offset_type get_raw_offset( offset_type point ) const {
		return round<typename raw_offset_type::data_type>( get_index_offset( point ) ) ;
	}
	...
	void shift_data_offset( const raw_offset_type& shift ){
		_byte_array += shift.y * _byte_row_step + shift.x;
	}
	...
	constexpr const shift_type width_step() const { return _byte_row_step; }
	constexpr const size_type total_raw_size() const {
		return ( _height > 0 && _width > 0) ? ( _height - 1 ) * _byte_row_step + sizeof(data_type) * _width : 0; }
	...
	constexpr data_type* row_data( const shift_type row )const {
		return static_cast<data_type*>(_byte_array + row * _byte_row_step);
	}
...
protected:
	bool revalidate_storage( const size_t width, const size_t height ){
		 ...
		 */
		const size_t fixed_width = ( ( ( width - 1 ) >> 4) + 1 ) << 4; //ceil to multiple of 1<<4;
...
//		cerr << storage_range << " " << test_data_pos << " + " << basic_image_signal<T>::total_size() << " = " << test_data_pos + basic_image_signal<T>::total_size() << "\n";
	...
	private:
	no_copy_data_holder<T> internal_array;
};
